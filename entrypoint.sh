...
case "$1" in
	'')
		...
		if [ "$DATABASE" == "" ]; then
			echo "If you want to create DBs automatically specify DATABASE environment variable";
		else
			if [ -f "/db.${DATABASE}" ];
			then
				echo "User ${DATABASE} already exists!!!"
			else
				echo "Is about to create database: $DATABASE"
				cp /template.schema.sql /$DATABASE.schema.sql
				sed -i -E "s/TEMPLATE_USER/$DATABASE/g" /$DATABASE.schema.sql
				sqlplus -S system/oracle@localhost @/$DATABASE.schema.sql
				touch "/db.${DATABASE}"

				if [ -f "/db-init/schema.sql" ];
				then
					echo "Is about to create schema"
					sqlplus -S system/oracle@localhost @/db-init/schema.sql
					touch "/db.${DATABASE}"
				fi

				if [ -f "/db-init/data.sql" ];
				then
					echo "Is about to insert data"
					sqlplus -S system/oracle@localhost @/db-init/data.sql
					touch "/db.${DATABASE}"
				fi
			fi
		fi
